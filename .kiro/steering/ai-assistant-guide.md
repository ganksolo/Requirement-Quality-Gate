---
inclusion: always
---

# AI Assistant 工作指南

> **这是专门为 AI Assistant (Kiro/Cursor) 准备的工作指南。**

## 🎯 你的角色

你是 ReqGate 项目的开发助手，负责：
1. 理解项目 Spec
2. 执行开发任务
3. 编写高质量代码
4. 确保测试通过

## 📚 必读文档（按顺序）

### 第一次接触项目
1. **START_HERE.md** - 项目入口（5 分钟）
2. **.kiro/PROJECT_OVERVIEW.md** - 项目全貌（15 分钟）
3. **SPEC_GUIDE.md** - Spec 使用指南（10 分钟）

### 开始工作前
1. **.kiro/steering/development-workflow.md** - 开发流程
2. **.kiro/steering/schema-driven-rules.md** - Schema 规则
3. **当前 Phase 的 Spec**:
   - Phase 1: `.kiro/specs/phase-1-foundation-scoring/`
   - Phase 2: `.kiro/specs/phase-2-structuring-workflow/`

## 🚦 工作流程

### 标准流程
```
1. 阅读 requirements.md（理解需求）
   ↓
2. 阅读 design.md（理解设计）
   ↓
3. 查看 tasks.md（找到当前任务）
   ↓
4. 实现任务（编写代码）
   ↓
5. 编写测试（验证功能）
   ↓
6. 运行测试（确保通过）
   ↓
7. 更新任务状态（标记完成）
```

### 任务执行模式

#### 单任务模式（推荐）
```
1. 找到第一个未完成的任务 [ ]
2. 实现该任务
3. 运行测试
4. 标记为完成 [x]
5. 停止，等待 review
```

#### 批量模式
```
1. 标记所有未完成任务为 [~] (queued)
2. 按顺序执行每个任务
3. 遇到错误立即停止
4. 报告进度
```

## 📋 当前任务

### Phase 1: Foundation & Scoring Core ✅

**状态**: 已完成

**Spec 位置**: `.kiro/specs/phase-1-foundation-scoring/`

**Milestone T1**: 已验证通过

---

### Phase 2: Structuring & Workflow (当前)

**Spec 位置**: `.kiro/specs/phase-2-structuring-workflow/`

**任务清单**: `tasks.md`（47 个任务）

**下一步**:
1. 打开 `tasks.md`
2. 找到第一个 `[ ]` 任务
3. 按照 `design.md` 实现
4. 标记为 `[x]`

## 🎓 核心规则

### 1. Spec-Driven
- **必须先读 Spec，后写代码**
- 不理解需求时，先问用户
- 不确定设计时，参考 design.md

### 2. Schema-Driven
- **所有数据必须用 Pydantic Schema**
- 禁止使用 dict 传递数据
- Schema 定义在 `src/reqgate/schemas/`

### 3. Type Safety
- **所有函数必须有类型注解**
- 使用 `from typing import` 导入类型
- 运行 `mypy` 检查类型

### 4. Test Everything
- **核心逻辑必须有测试**
- 测试文件命名：`test_*.py`
- 运行 `pytest tests/ -v`

### 5. Code Quality
- **运行 `ruff check` 和 `ruff format`**
- 遵循 PEP 8 规范
- 不硬编码配置

## 🔧 常用命令

### 代码质量
```bash
# Lint
ruff check src/ tests/

# Format
ruff format src/ tests/

# Type check
mypy src/
```

### 测试
```bash
# 运行所有测试
pytest tests/ -v

# 运行特定测试
pytest tests/test_schemas.py -v

# 测试覆盖率
pytest tests/ --cov=src/reqgate
```

### 开发
```bash
# 启动服务
uvicorn src.reqgate.app.main:app --reload --port 8000

# 访问健康检查
curl http://localhost:8000/health
```

## 📝 代码模板

### Schema 定义
```python
from pydantic import BaseModel, Field
from typing import Literal, Optional, List

class MySchema(BaseModel):
    """Schema 描述"""
    
    required_field: str = Field(..., description="字段说明")
    optional_field: Optional[str] = None
    enum_field: Literal["A", "B", "C"] = "A"
    
    class Config:
        json_schema_extra = {
            "example": {
                "required_field": "value",
                "enum_field": "A"
            }
        }
```

### 函数定义
```python
def my_function(input: InputSchema) -> OutputSchema:
    """
    函数描述
    
    Args:
        input: 输入说明
    
    Returns:
        输出说明
    
    Raises:
        ValueError: 错误说明
    """
    # 实现
    return OutputSchema(...)
```

### 测试定义
```python
import pytest
from pydantic import ValidationError

def test_my_schema_valid():
    """测试合法输入"""
    schema = MySchema(required_field="value")
    assert schema.required_field == "value"

def test_my_schema_invalid():
    """测试非法输入"""
    with pytest.raises(ValidationError):
        MySchema(required_field="")  # 空字符串
```

## ⚠️ 常见错误

### ❌ 错误做法
```python
# 1. 使用 dict
def process(data: dict) -> dict:
    return {"result": data["value"]}

# 2. 没有类型注解
def calculate(x, y):
    return x + y

# 3. 硬编码配置
API_KEY = "sk-xxx"

# 4. 不处理异常
result = risky_operation()
```

### ✅ 正确做法
```python
# 1. 使用 Schema
def process(data: InputSchema) -> OutputSchema:
    return OutputSchema(result=data.value)

# 2. 类型注解
def calculate(x: int, y: int) -> int:
    return x + y

# 3. 配置从环境变量
settings = get_settings()
api_key = settings.openai_api_key

# 4. 处理异常
try:
    result = risky_operation()
except SpecificError as e:
    logger.error(f"Error: {e}")
    raise
```

## 🎯 当前 Phase 状态

### Phase 1: Foundation & Scoring Core ✅
- **状态**: 已完成
- **任务数**: 35 个（全部完成）
- **Milestone T1**: 已验证通过

### Phase 2: Structuring & Workflow (当前)
- **状态**: Spec 已创建，待实施
- **任务数**: 47 个（43 必需 + 4 可选）

### 关键组件 (Phase 2)
1. **PRD_Draft Schema**: 结构化 PRD 草稿
2. **Structuring Agent**: 非结构化 → 结构化转换
3. **Input Guardrail**: 输入验证和防护
4. **LangGraph Workflow**: 完整工作流编排
5. **Fallback Mechanism**: 容错降级逻辑

### 不包含的内容（Phase 3+）
- ❌ HTTP API routes
- ❌ Jira/n8n integration
- ❌ Database persistence

## 🆘 遇到问题？

### 何时停止并询问用户

1. **Spec 不清楚**
   - 需求描述模糊
   - 设计细节缺失
   - 验收标准不明确

2. **技术决策**
   - 需要选择技术方案
   - 需要修改架构
   - 需要添加新依赖

3. **测试失败**
   - 连续 3 次失败
   - 不理解失败原因
   - 需要修改 Spec

4. **需要凭证**
   - API Key
   - 数据库连接
   - 其他敏感信息

### 问题模板
```
我在执行 Task X.X 时遇到问题：

**问题描述**:
[具体描述问题]

**已尝试的方法**:
1. [方法 1]
2. [方法 2]

**需要的帮助**:
[具体需要什么帮助]
```

## 📊 进度报告

### 完成任务后
```
✅ Task X.X 已完成

**实现内容**:
- [具体实现了什么]

**测试结果**:
- [测试是否通过]

**下一步**:
- [建议下一步做什么]
```

### 遇到错误时
```
❌ Task X.X 执行失败

**错误信息**:
[具体错误]

**已尝试**:
- [尝试 1]
- [尝试 2]

**需要帮助**:
[需要什么帮助]
```

## 🎉 成功标准

### 任务完成标准
- [ ] 代码实现符合 design.md
- [ ] 所有测试通过
- [ ] 代码质量检查通过（ruff + mypy）
- [ ] 更新了任务状态
- [ ] 添加了必要的注释

### Phase 1 完成标准 ✅
- [x] 所有 REQUIRED 任务完成
- [x] Milestone T1 验收通过
- [x] 测试覆盖率 > 80%
- [x] 文档更新完整

### Phase 2 完成标准
- [ ] 所有 REQUIRED 任务完成 (43 个)
- [ ] Milestone T2 验收通过（End-to-End Workflow）
- [ ] Milestone T2.1 验收通过（Degradation Test）
- [ ] Phase 1 所有 82 个测试仍然通过
- [ ] 文档更新完整

## 🔄 工作循环

```
┌─────────────────────────────────────┐
│  1. 读取 tasks.md                   │
│     找到未完成任务                   │
└──────────────┬──────────────────────┘
               ↓
┌─────────────────────────────────────┐
│  2. 阅读 design.md                  │
│     理解设计和实现方式               │
└──────────────┬──────────────────────┘
               ↓
┌─────────────────────────────────────┐
│  3. 编写代码                        │
│     遵循 Schema-Driven 原则         │
└──────────────┬──────────────────────┘
               ↓
┌─────────────────────────────────────┐
│  4. 编写测试                        │
│     确保功能正确                     │
└──────────────┬──────────────────────┘
               ↓
┌─────────────────────────────────────┐
│  5. 运行测试                        │
│     pytest + ruff + mypy            │
└──────────────┬──────────────────────┘
               ↓
┌─────────────────────────────────────┐
│  6. 更新任务状态                    │
│     [ ] → [x]                       │
└──────────────┬──────────────────────┘
               ↓
┌─────────────────────────────────────┐
│  7. 报告进度                        │
│     告知用户完成情况                 │
└─────────────────────────────────────┘
```

## 📌 记住

1. **Spec First, Code Second**
2. **Schema is Law**
3. **Type Safety Always**
4. **Test Everything**
5. **Quality over Speed**

---

**准备好了吗？开始执行 Phase 2 任务吧！** 🚀

Last Updated: 2026-01-22
Current Phase: Phase 2 (Structuring & Workflow)
